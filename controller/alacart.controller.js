    ...
    try {
        if (req.session && req.session.Plan_UserName && req.session.Plan_StoreId) {
            mysql.getConnection('CMS', function (err, connection_ikon_cms) {
                if(err){
                    logger.writeLog('getalacartadata : '+ JSON.stringify(err));
                }else {
                    async.parallel({
                    ...
                    function (err, results) {
                            if (err) {
                                ...
                                connection_ikon_cms.release();
                                logger.writeLog('getalacartadata : '+ JSON.stringify(results));
                                res.send(results);
                            }
                        });
                }
...
exports.addeditalacart = function (req, res, next) {
    try {
        if (req.session && req.session.Plan_UserName && req.session.Plan_StoreId) {
            mysql.getConnection('CMS', function (err, connection_ikon_cms) {
                if(err){
                    logger.writeLog('addeditalacart : '+ JSON.stringify(err));
                }else {
                    async.waterfall([
                            function (callback) {
                            ...
                            if (results.message) {
                                ...
                                function EditALacart() {
                                    async.waterfall([
                                        ...
                                        function (err, results) {
                                            if (err) {
                                                logger.writeLog('EditALacart : ' + JSON.stringify(err));
                                                connection_ikon_cms.release();
                                                res.status(500).json(err.message);
                                            } else {
                                                var data = {
                                                ...
                                                alacartaManager.updateIcnAlacartPlan(connection_ikon_cms, data, req.body.alacartplanid, function (err, result) {
                                                    if (err) {
                                                        ...
                                                        connection_ikon_cms.release();
                                                        logger.writeLog('EditALacart : ' + JSON.stringify('A-La-Cart Plan Updated successfully'));
                                                        res.send({
                                                            success: true,
                                ...
                                function AddAlacart() {
                                    async.waterfall([
                                        ...
                                        function (err, results) {
                                            if (err) {
                                                logger.writeLog('AddAlacart : ' + JSON.stringify(err));
                                                connection_ikon_cms.release();
                                                res.status(500).json(err.message);
                                            } else {
                                                var data = {
                                                ...
                                                alacartaManager.createIcnAlacartPlan(connection_ikon_cms, data, function (err, result) {
                                                    if (err) {
                                                        ...
                                                        connection_ikon_cms.release();
                                                        logger.writeLog('AddAlacart : ' + JSON.stringify('A-La-Cart Plan added successfully.'));
                                                        res.send({
                                                            success: true,
